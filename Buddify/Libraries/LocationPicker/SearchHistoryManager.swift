...
struct SearchHistoryManager {
	...
	func history() -> [DiscoverySearchLocation] {
		let history = defaults.objectForKey(HistoryKey) as? [NSDictionary] ?? []
		return history.flatMap(DiscoverySearchLocation.fromDefaultsDic)
	}
	
	func addToHistory(location: DiscoverySearchLocation) {
...
extension DiscoverySearchLocation {
	func toDefaultsDic() -> NSDictionary? {
	...
	class func fromDefaultsDic(dic: NSDictionary) -> DiscoverySearchLocation? {
		...
		return DiscoverySearchLocation(name: dic[LocationDicKeys.name] as? String,
			location: location,
